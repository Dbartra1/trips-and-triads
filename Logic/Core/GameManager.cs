	...
	public class GameManager
	{
		...
		public void DealHands(List<CardData> player1Cards, List<CardData> player2Cards)
		{
			if (Config.Conscription)
			{
				...
				TestLogger.Log("Conscription active — P1 hand drawn randomly from roster.");
			}
			else
			{
				...
				foreach (var card in player2Cards)
				{
					...
					if (card.AbilityType == AbilityType.Decay && card.Tier == Tier.Hero)
					{
						...
						TestLogger.Log($"Vesna enters at {cap}/{cap}/{cap}/{cap}.");
					}
			...
			if (Config.Intercept)
				TestLogger.Log("Intercept active — both hands are open.");

			TestLogger.Log($"Hands dealt. P1: {_hands[1].Count} cards, P2: {_hands[2].Count} cards.");
		}
		...
		public List<(int row, int col)>? PlayCard(int handIndex, int row, int col)
		{
			if (GameOver)
			{
				TestLogger.Error("GameManager: game is already over.");
				return null;
			}
			...
			{
				TestLogger.Error($"GameManager: invalid hand index {handIndex}.");
				return null;
			}

			if (!Board.IsEmpty(row, col))
			{
				TestLogger.Error($"GameManager: cell ({row},{col}) is already occupied.");
				return null;
			}
			...
			TestLogger.Log($"P{CurrentPlayerId} played {card.Data.Name} at ({row},{col}). " +
			...
			for (int r = 0; r < BoardState.Size; r++)
				for (int c = 0; c < BoardState.Size; c++)
				{
					...
					foreach (Direction dir in System.Enum.GetValues(typeof(Direction)))
					{
						...
						{
							TestLogger.Log($"{card.Data.Name} flipped by decay — " +
		...
		private void EndGame()
		{
			...
			{
				TestLogger.Log("Standoff — draw! Rematch with board-state hands.");
				StandoffTriggered = true;
				return;
			}

			GameOver = true;
			TestLogger.Log($"Game Over! P1: {p1Score} | P2: {p2Score}");

			if (p1Score > p2Score)      TestLogger.Log("Player 1 wins!");
			else if (p2Score > p1Score) TestLogger.Log("Player 2 wins!");
			else                        TestLogger.Log("It's a draw!");
		}

		public void PrintScores()
		{
			TestLogger.Log($"Scores — P1: {Board.GetScore(1)} | P2: {Board.GetScore(2)}");
		}
	}
}